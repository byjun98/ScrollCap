...
export async function putAssetFromDataUrl(assetId: string, dataUrl: string, meta: any = {}) {
  const blob = await dataUrlToBlob(dataUrl);
  return putAssetBlob(assetId, blob, meta);
}

export async function putAssetBlob(assetId: string, blob: Blob, meta: any = {}) {
  ...
  const record = {
    ...
    byteLength: meta.byteLength ?? blob.size ?? null,
    createdAt: timestamp,
